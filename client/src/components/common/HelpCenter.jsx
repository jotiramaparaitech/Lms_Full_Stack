    ...
    items: [
      {
        ...
        answer: "Upon enrollment, you will receive a Project PDF with all the details. You will also be added to a dedicated WhatsApp group with other participants."
      },
      {
        question: "How often are the mentorship meetings?",
        answer: "We conduct live meetings 2-3 times a week. These sessions are for tracking progress, solving doubts, and reviewing your code."
      },
      {
        question: "Where can I ask day-to-day doubts?",
        answer: "Please post your queries in the assigned WhatsApp group. Mentors and peers are active there to help you immediately."
      },
      {
        question: "When do I get my certificate?",
        answer: "Once you complete the project and it is approved by the mentor during the final review, your certificate will be issued to you."
      }
    ]
  }
];
...
const HelpCenter = ({ onClose }) => {
  ...
  const handleChat = () => {
    const message = "Hi, I need help with the LMS.";
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
      ...
      <div className="bg-white w-full max-w-[500px] rounded-3xl shadow-2xl overflow-hidden flex flex-col relative h-[700px] max-h-[90vh] animate-in zoom-in-95 duration-200 font-sans ring-1 ring-black/5">
              ...
              {FAQS.map((category, catIndex) => (
                <div key={catIndex}>
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                    {catIndex === 0 ? <FileText className="w-3 h-3" /> : <MonitorPlay className="w-3 h-3" />}
                    {category.category}
                  </h3>
                  <div className="space-y-2">
                    {category.items.map((faq, itemIndex) => {
                      ...
                      return (
                        <div 
                          key={itemIndex} 
                          className={`bg-white border rounded-xl transition-all duration-200 ${isOpen ? 'border-blue-200 shadow-md ring-1 ring-blue-50' : 'border-gray-200 shadow-sm hover:border-blue-300'}`}
                        >
                          <button
                            onClick={() => toggleFaq(id)}
                            className="w-full flex items-start justify-between p-4 text-left"
                          >
                            <span className={`text-[15px] font-medium leading-tight ${isOpen ? 'text-blue-700' : 'text-slate-700'}`}>
                              {faq.question}
                            </span>
                            <span className={`ml-3 mt-0.5 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}>
                              <ChevronDown className={`w-5 h-5 ${isOpen ? 'text-blue-500' : 'text-gray-300'}`} />
                            </span>
                          </button>
          ...
          >
            ...
            <div className="mt-8 p-4 bg-slate-50 rounded-xl border border-slate-100 text-center">
              <p className="text-xs text-slate-400">
                Experiencing urgent technical issues with a live project submission? 
                <br/> Email us directly at <span className="text-slate-600 font-medium">support@aparaitech.org</span>
              </p>
            </div>
          </div>
        ...
        {/* === FOOTER (Sticky) === */}
        <div className={`p-4 bg-white border-t border-gray-100 absolute bottom-0 w-full transition-transform duration-300 ${view === "CONTACT_MENU" ? 'translate-y-full' : 'translate-y-0'}`}>
          <button
...
const ContactOption = ({ icon, color, title, desc, onClick }) => (
  ...
  >
    <div className={`w-12 h-12 ${color} rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform`}>
      {icon}
    </div>
